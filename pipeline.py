...
class OrthoPipeline:
    ...
    def generate_orthomosaic(self) -> Tuple[np.ndarray, tuple, float]:
        ...
        for i, (m, pose) in enumerate(zip(meta, poses)):
            ...
            # Build camera for this image
            cam  = self._build_camera(m)

            # Pre-undistort the image if the camera model supports it.
            # Using an undistorted image with a pinhole camera avoids radial swirl artifacts.
            if hasattr(cam, 'undistort_image'):
                # Undistort using OpenCV maps (Brown–Conrady/Equidistant camera)
                undist_img = cam.undistort_image(img)
                # Create a pinhole camera with identical intrinsics but no distortion
            ...
            # Color harmonization on the image (after undistortion if applied)
            img_f = img_use.astype(np.float64)
            for c in range(3):
                # Compute per-channel mean/std on non-zero pixels
                mask_c = img_f[:,:,c] > 0
            ...
            # Orthorectify using the distortion-free camera
        ...
        if fill_meta and len(fill_meta) > len(meta):
            ...
            if fill_set:
                ...
                for fi, (m, pose) in enumerate(fill_set):
                    ...
                    cam = self._build_camera(m)
                    # Pre-undistort if supported and use pinhole camera for projection
                    if hasattr(cam, 'undistort_image'):
                        undist_img = cam.undistort_image(img)
                        pin_cam = PinholeCamera(
                            width=undist_img.shape[1],
